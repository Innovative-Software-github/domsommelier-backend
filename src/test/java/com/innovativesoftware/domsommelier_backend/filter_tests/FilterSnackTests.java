...
@DisplayName("Тесты для фильтра снеков")
public class FilterSnackTests {
    ...
    private List<Map<String, Object>> filterProducts(Map<String, Object> params) throws Exception {
        ...
        MvcResult mvcResult = this.mockMvc.perform(
                        post("/api/v1/products/filter")
                                .param("category", String.valueOf(ProductCategoryEnum.snack))
                                .param("page", "0")
                                .param("size", "10")
    ...
    @DisplayName("Тестирование поля назначений (pairings)")
    @Test
    void testFilterPairings() throws Exception {
        for (String feature : List.of("для Белого вина", "для Красного вина")) {
            Map<String, Object> params = new HashMap<>();
            params.put("pairings", List.of(feature));
            List<Map<String, Object>> resultList = filterProducts(params);
            ...
            resultList.forEach(
                    product -> {
                        try {
                            ...
                            assertNotNull(productDTO);
                            Map<String, Object> details = (Map<String, Object>) productDTO.getDetails();
                            assertTrue(details.containsKey("pairings"));
                            List<String> features = (List<String>) details.get("pairings");
