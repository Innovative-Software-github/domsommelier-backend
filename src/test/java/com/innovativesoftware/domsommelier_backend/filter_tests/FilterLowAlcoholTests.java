...
@DisplayName("Тесты для фильтра лёгкого алкоголя")
public class FilterLowAlcoholTests {
    ...
    private List<Map<String, Object>> filterProducts(Map<String, Object> params) throws Exception {
        ...
        MvcResult mvcResult = this.mockMvc.perform(
                        post("/api/v1/products/filter")
                                .param("category", String.valueOf(ProductCategoryEnum.low_alcohol))
                                .param("page", "0")
                                .param("size", "10")
    ...
    void testFilterPrice() throws Exception {
        ...
        params.put("price", List.of(200, 300));
        ...
        resultList.forEach(
                product -> {
                    ...
                    assertTrue((Double) product.get("price") >= 200 && (Double) product.get("price") <= 300);
                });
    }
    ...
    void testFilterCountry() throws Exception {
        for (String country : List.of("Германия", "Франция")) {
            Map<String, Object> params = new HashMap<>();
    ...
    void testFilterProducer() throws Exception {
        for (String producer : List.of("Green Apple Brewery", "Seltzeria")) {
            Map<String, Object> params = new HashMap<>();
    ...
    @DisplayName("Тестирование субкатегории")
    @Test
    void testFilterCategory() throws Exception {
        for (String feature : List.of("cider", "seltzer")) {
            Map<String, Object> params = new HashMap<>();
    ...
    void testFilterVolume() throws Exception {
        for (BigDecimal volume : List.of(BigDecimal.valueOf(0.50), BigDecimal.valueOf(0.33))) {
            Map<String, Object> params = new HashMap<>();
    ...
    void testFilterStrength() throws Exception {
        for (BigDecimal strength : List.of(BigDecimal.valueOf(5.00), BigDecimal.valueOf(2.50))) {
            Map<String, Object> params = new HashMap<>();
    ...
    void testFilterFeatures() throws Exception {
        for (String feature : List.of("Свежий яблочный вкус", "Освежающий эффект")) {
            Map<String, Object> params = new HashMap<>();
